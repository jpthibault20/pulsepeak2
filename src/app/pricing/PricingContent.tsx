...
export function PricingContent({ currentPlan = 'free' }: { currentPlan?: Plan }) {
                    ...
                    {plans.map(plan => {
                        ...
                        return (
                            ...
                            >
                                ...
                                {/* Badge recommandé (seulement si pas le plan actuel) */}
                                ...
                                <div className="flex items-center gap-2 mb-2">
                                    <span className="text-slate-900 dark:text-white font-bold text-lg">{plan.name}</span>
                                    {plan.badge && (
                                    ...
                                    {!plan.available && plan.id === 'pro' && (
                                        <Lock size={12} className="text-slate-500 dark:text-slate-500" />
                                    )}
                                </div>
                                ...
                                <div className="mb-3">
                                    ...
                                    <span className="text-slate-500 dark:text-slate-400 text-sm ml-1">{plan.priceDetail}</span>
                                </div>

                                {/* Description */}
                                <p className="text-slate-500 dark:text-slate-400 text-sm leading-relaxed mb-5">{plan.description}</p>
                                ...
                                {isCurrent ? (
                                    ...
                                    <>
                                        ...
                                        {plan.id === 'pro' && (
                                            <button
                                                disabled
                                                className="flex items-center justify-center gap-2 bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-500 font-semibold py-3 rounded-xl text-sm cursor-not-allowed border border-slate-300 dark:border-slate-700 w-full"
                                            >
                                                <Crown size={14} />
                                                {plan.cta}
                                            </button>
