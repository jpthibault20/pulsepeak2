    ...
    return (
        <>
            ...
            <div className="md:hidden fixed bottom-3 left-3 right-3 z-50">
                {/* Pill container */}
                <div className="relative bg-slate-900/95 backdrop-blur-xl border border-white/[0.08] rounded-2xl shadow-2xl shadow-black/60 h-16 flex items-center px-1">
...
const MobileItem: React.FC<ItemProps> = ({ active, onClick, icon: Icon, label }) => (
    ...
    >
        <Icon
            ...
            className={`transition-all duration-200 ${active ? 'text-blue-400' : 'text-slate-500'}`}
        />
        <span className={`text-[10px] font-medium transition-colors duration-200 ${active ? 'text-blue-400' : 'text-slate-600'}`}>
            {label}
        </span>
        {active && (
