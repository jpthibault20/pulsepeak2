...
import {
    User, Clock, Calculator, Target, Save, Zap, TrendingUp
} from 'lucide-react';
...
export const ProfileForm: React.FC<ProfileFormProps> = ({ initialProfileData, isSettings = false, onSave, onSuccess, onCancel }) => {
    ...
    const calculateZones = () => {
        ...
        // Préparation des données pour le modèle Critical Power
        // On convertit en secondes et en Joules (Puissance * Temps)
        ...
        // 1. Algorithme "Directeur Sportif"
        if (dataPoints.length >= 2) {
            // Si on a au moins 2 points, on utilise la régression linéaire (Modèle CP)
            // Work = CP * Time + W'
            // C'est la méthode la plus précise scientifiquement
            ...
            // Calcul de la pente (CP) et de l'ordonnée à l'origine (W')
            const slope = (n * sumTW - sumT * sumW) / (n * sumT2 - sumT * sumT);
            const intercept = (sumW - slope * sumT) / n;

            estimatedFtp = Math.round(slope);
            wPrime = Math.round(intercept); // En Joules

            sourceUsed = `Modèle Puissance Critique (${testsUsed.join('+')})`;
            methodUsed = 'Critical Power Regression';

        } else {
            // Fallback : Méthode classique avec un seul point
            if (p20 > 0) {
                estimatedFtp = Math.round(p20 * 0.95);
                sourceUsed = '95% du CP20';
            } else if (p15 > 0) {
                estimatedFtp = Math.round(p15 * 0.93);
                sourceUsed = '93% du CP15';
            } else if (p8 > 0) {
                estimatedFtp = Math.round(p8 * 0.90);
                sourceUsed = '90% du CP8';
            } else if (p5 > 0) {
                estimatedFtp = Math.round(p5 * 0.82);
                sourceUsed = '82% du CP5 (Estimatif)';
            } else {
                alert("Veuillez entrer au moins une valeur de test (5, 8, 15 ou 20 min) pour calculer.");
                return;
            }
            methodUsed = 'Single Test Estimation';
        }

        // 2. Calcul des Zones de Coggan (7 Zones)
        const newZones: PowerZones = {
            ...
            // Z6 plafonnée par la PMA réelle (p5) si connue, sinon estimation 150% FTP
        ...
        <div className={`space-y-6 ${isSettings ? 'pb-20' : ''}`}>
            {!isSettings && (
                <div className="text-center mb-10">
                    ...
                    <p className="text-slate-400">Profil physiologique & Zones de puissance</p>
                </div>
            )}
            ...
            <Card>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div>
                        ...
                        <input type="text" value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} className="w-full bg-slate-900 border border-slate-700 rounded-lg p-3 text-white focus:ring-2 focus:ring-blue-500 outline-none" />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-slate-400 mb-1">Poids (kg)</label>
                        <input type="number" value={formData.weight} onChange={e => setFormData({ ...formData, weight: parseInt(e.target.value) })} className="w-full bg-slate-900 border border-slate-700 rounded-lg p-3 text-white focus:ring-2 focus:ring-blue-500 outline-none" />
                    </div>
                </div>
                ...
                <div className="bg-slate-900/50 p-4 rounded-xl border border-slate-700 mb-6">
                    <div className="flex justify-between items-center mb-4">
                        ...
                        {formData.ftp > 0 && <span className="text-xs text-slate-400">FTP Actuelle: {formData.ftp}W</span>}
                    </div>

                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                        <div>
                            <label className="block text-xs text-slate-400 mb-1 text-center">5 min (PMA)</label>
                            <input
                                type="number"
                                value={formData.powerTests?.p5min || ''}
                                onChange={e => handleTestChange('p5min', e.target.value)}
                                className="w-full bg-slate-800 border border-slate-600 rounded p-2 text-white text-center focus:border-blue-500 outline-none"
                                placeholder="---"
                            />
                        </div>
                        <div>
                            <label className="block text-xs text-slate-400 mb-1 text-center">8 min</label>
                            <input
                                type="number"
                                value={formData.powerTests?.p8min || ''}
                                onChange={e => handleTestChange('p8min', e.target.value)}
                                className="w-full bg-slate-800 border border-slate-600 rounded p-2 text-white text-center focus:border-blue-500 outline-none"
                                placeholder="---"
                            />
                        </div>
                        <div>
                            <label className="block text-xs text-slate-400 mb-1 text-center">15 min</label>
                            <input
                                type="number"
                                value={formData.powerTests?.p15min || ''}
                                onChange={e => handleTestChange('p15min', e.target.value)}
                                className="w-full bg-slate-800 border border-slate-600 rounded p-2 text-white text-center focus:border-blue-500 outline-none"
                                placeholder="---"
                            />
                        </div>
                        <div>
                            <label className="block text-xs text-slate-400 mb-1 text-center">20 min (FTP)</label>
                            <input
                                type="number"
                                value={formData.powerTests?.p20min || ''}
                                onChange={e => handleTestChange('p20min', e.target.value)}
                                className="w-full bg-slate-800 border border-slate-600 rounded p-2 text-white text-center focus:border-blue-500 outline-none border-l-2 border-l-blue-500"
                                placeholder="---"
                            />
                        </div>
                    </div>

                    <Button variant="secondary" onClick={calculateZones} className="w-full py-2 text-sm mb-4" icon={Calculator}>
                        Calculer FTP & Zones
                    </Button>
                        ...
                        <div className="mt-4 space-y-3 animate-in fade-in slide-in-from-top-2 pt-4 border-t border-slate-700">
                            ...
                            {formData.seasonData?.wPrime && formData.seasonData.wPrime > 0 && (
                                <div className="flex items-center gap-2 bg-slate-800/50 p-2 rounded mb-2 text-xs text-slate-400 border border-slate-600/30">
                                    ...
                                    <span>Réserve Anaérobie (W&apos;): <span className="text-orange-300 font-mono">{formData.seasonData.wPrime} J</span></span>
                                </div>
                            )}
                            ...
                            <div className="grid grid-cols-7 gap-1 text-center">
                                {[
                                    ...
                                    <div key={zone.label} className={`bg-slate-800/40 p-1.5 rounded border-t-2 ${zone.color.split(' ')[0]}`}>
                                        <div className={`font-bold text-xs ${zone.color.split(' ')[1]}`}>{zone.label}</div>
                                        <div className="text-[10px] text-slate-500 uppercase mb-1 hidden sm:block">{zone.name}</div>
                ...
                <div className="bg-slate-900/50 p-4 rounded-xl border border-slate-700 space-y-4 mb-6">
                    {['Lundi', 'Mardi', 'Mercredi', 'Jeudi', 'Vendredi', 'Samedi', 'Dimanche'].map(day => (
                        <div key={day} className="flex items-center justify-between">
                            ...
                            <div className="flex-1 mx-4">
                                <input type="range" min="0" max="300" step="30" value={formData.weeklyAvailability[day] || 0} onChange={(e) => handleAvailabilityChange(day, e.target.value)} className="w-full h-2 bg-blue-600/30 rounded-lg appearance-none cursor-pointer" style={{ background: `linear-gradient(to right, #3b82f6 0%, #3b82f6 ${((formData.weeklyAvailability[day] || 0) / 300) * 100}%, #475569 ${((formData.weeklyAvailability[day] || 0) / 300) * 100}%, #475569 100%)` }} />
                            </div>
                    ...
                    <div className="mt-4 pt-4 border-t border-slate-700 flex items-center justify-between bg-slate-800/50 p-3 rounded-lg">
                        <div className="flex items-center text-slate-300"><Calculator size={18} className="mr-2 text-blue-400" /><span className="font-medium text-sm">Volume Total Possible :</span></div>
                ...
                <div className="space-y-4 mb-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <select value={formData.goal} onChange={e => setFormData({ ...formData, goal: e.target.value })} className="w-full bg-slate-900 border border-slate-700 rounded-lg p-3 text-white focus:ring-2 focus:ring-blue-500 outline-none">
                        ...
                        <div>
                            ...
                            <input type="date" value={formData.objectiveDate} onChange={e => setFormData({ ...formData, objectiveDate: e.target.value })} className="w-full bg-slate-900 border border-slate-700 rounded-lg p-3 text-white focus:ring-2 focus:ring-blue-500 outline-none" />
                        </div>
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-slate-400 mb-1">Points Faibles / Commentaires IA</label>
                        <textarea value={formData.weaknesses} onChange={e => setFormData({ ...formData, weaknesses: e.target.value })} className="w-full bg-slate-900 border border-slate-700 rounded-lg p-3 text-white focus:ring-2 focus:ring-blue-500 outline-none h-20 resize-none" />
                    </div>
                </div>
