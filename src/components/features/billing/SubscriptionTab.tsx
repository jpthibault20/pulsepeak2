        ...
        <div className="space-y-6">
            ...
            <div className="bg-slate-800/60 border border-slate-700 rounded-2xl p-5">
                <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                        <p className="text-white font-semibold text-sm">Abonnement actuel</p>
                    ...
                    <span className={`
                        ...
                        ${currentPlan === 'free' ? 'bg-slate-700 text-slate-300 border-slate-600' : ''}
                        ${currentPlan === 'dev'  ? 'bg-amber-500/20 text-amber-300 border-amber-500/30' : ''}
                        ${currentPlan === 'pro'  ? 'bg-blue-500/20 text-blue-300 border-blue-500/30' : ''}
                    `}>
                                    ...
                                    <ul className="space-y-1.5">
                                        {plan.features.map(f => (
                                            <li key={f} className="flex items-center gap-2 text-xs text-slate-400">
                                                <Check size={11} className={isLocked ? 'text-slate-600' : 'text-emerald-400'} />
                                                {f}
                                            </li>
                                        ))}
                                    </ul>
                            ...
                            {!isCurrent && (
                                <div className="mt-4 pt-4 border-t border-slate-700/50">
                                    ...
                                    {plan.id === 'pro' && (
                                        <button
                                            disabled
                                            className="w-full flex items-center justify-center gap-2 bg-slate-700 text-slate-500 font-semibold py-2.5 rounded-xl text-sm cursor-not-allowed border border-slate-600"
                                        >
                                            <Crown size={14} />
                                            {plan.cta}
                                        </button>
                                    ...
                                    {plan.id === 'free' && currentPlan !== 'free' && (
                                        <button
                                            disabled
                                            className="w-full flex items-center justify-center gap-2 bg-slate-800 text-slate-500 font-semibold py-2.5 rounded-xl text-sm cursor-not-allowed"
                                        >
                                            Rétrograder
                                        </button>
                                    )}
            ...
            <div className="bg-slate-800/40 border border-slate-700/50 rounded-xl p-4">
                <p className="text-slate-500 text-xs leading-relaxed text-center">
                    ...
                    <span className="block mt-1 text-slate-600">Intégration paiement Stripe en cours.</span>
                </p>
            </div>
        </div>
    );
